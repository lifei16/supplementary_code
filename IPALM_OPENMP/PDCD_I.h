...
/*
The optimization problem to solve is of the form min f(x)+g(x) under the constraint Mx=c;
...
// sample S
  ...
  inline void compute_function_value() {
     ...
     function_value=res;
     for(L j=0;j<m_2;j++){
        res2+=(Mx[j]-data_M.b[j])*(Mx[j]-data_M.b[j]);
     }
     residual= sqrt(res2);
   }
  ...
  inline void compute_and_record_res(){
        ...
        cout<<setprecision(9)<<"Iteration: "<<nb_iters<<"; time="<<running_time<<"; infeasibility= "<< residual<<"; function value="<<function_value<< endl;
        samp<<setprecision(9)<< nb_iters<<" "<<running_time<<" "<<residual<< " "<< function_value<<" "<<endl;
        }
   }
  ...
  inline D partial_i_of_f(L i)
  {
      ...
      for (L k = data_M.ptr_t[i]; k < data_M.ptr_t[i + 1];k++)
      {
        ...
        D tmp=(gamma*Mu[j]+Mz[j]-data_M.b[j])/beta_s+lambda[j];
  ...
  void PDCD_I_solver(D beta_0, vector<D> & x0,vector<D> & y0, L val_tau, L max_nb_epoch, L p_N, D val_lambda_f,string filename1, D time2)
  {
    initialize(beta_0, x0, y0, val_tau,  p_N, val_lambda_f);
    cout<<"running SMART_CD"<<" ; "<<filename1<<" max_nb_epoch "<<max_nb_epoch<<endl;
    nb_iters=0;
    filename1="results/PDCD_I_"+filename1;
